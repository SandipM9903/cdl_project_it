...
// Move Development Goal Modal outside main component to prevent re-renders
const DevelopmentGoalModal = ({
  isOpen,
  onClose,
  onSave,
  editingGoal,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-fadeIn">
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            {devErrors.description && (
              <p className="text-xs text-red-500 mt-1 flex items-center gap-1">
                <FaInfoCircle /> {devErrors.description}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
          ...
          >
            <FaSave size={14} />
            {editingGoal ? "Update" : "Save"} Development Goal
          </button>
        </div>
      </div>
    </div>
...
const EmployeeGoalCreation = () => {
  ...
  // Development Goals dropdown options
  const titleOptions = [
  ...
  const fetchExistingGoals = async () => {
    try {
      ...
      if (response.data && response.data.data) {
        existingGoals = response.data.data;
      } else if (Array.isArray(response.data)) {
        existingGoals = response.data;
      }
      ...
      if (existingGoals.length > 0) {
        const formattedGoals = existingGoals.map((goal) => {
          ...
          if (
            goal.target &&
            targetOperatorOptions.includes(goal.target.trim())
          ) {
            targetOperator = goal.target.trim();
          }
      ...
      console.error("Error fetching existing SMART goals:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchDevelopmentGoals = async () => {
    try {
      ...
      let devGoals = [];
      if (response.data && response.data.data) {
        devGoals = response.data.data;
      } else if (Array.isArray(response.data)) {
        devGoals = response.data;
      }
  ...
  const removeGoal = (index) => {
    if (goals.length === 1) {
      showModal(
        "Cannot Remove",
        "You must have at least one SMART goal",
        "warning",
      );
      return;
    }
  ...
  const validateDevGoal = () => {
    ...
    if (!devGoalForm.trainingName)
      newErrors.trainingName = "Training name is required";
    if (!devGoalForm.description.trim())
  ...
  const saveDevelopmentGoal = async () => {
    ...
    const payload = {
      ...
      selfAssessmentScore: devGoalForm.selfAssessment
        ? parseInt(devGoalForm.selfAssessment)
        : null,
    };
    ...
    try {
      if (editingDevGoal && editingDevGoal.id) {
        await axios.put(
          `${BASE_URL_EPMS}/api/development-goals/update/${editingDevGoal.id}`,
          payload,
        );
        showModal(
          "Success",
          "Development goal updated successfully!",
          "success",
          () => {
            closeModal();
          },
        ...
        await axios.post(
          `${BASE_URL_EPMS}/api/development-goals/create/${selectedQuarter}`,
          payload,
        );
        showModal(
          "Success",
          "Development goal added successfully!",
          "success",
          () => {
            closeModal();
          },
      ...
      console.error("Error saving development goal:", err);
      showModal(
        "Error",
        err.response?.data?.message || "Failed to save development goal",
        "error",
      );
    }
  };
  ...
  const deleteDevelopmentGoal = async (goalId) => {
    showModal(
      "Confirm Delete",
      "Are you sure you want to delete this development goal?",
      "warning",
      async () => {
        try {
          await axios.delete(
            `${BASE_URL_EPMS}/api/development-goals/${goalId}`,
          );
          showModal(
            "Success",
            "Development goal deleted successfully!",
            "success",
            () => {
              closeModal();
            },
  ...
  // SMART Goals save functions
  const calculateTotalWeightage = () => {
    return goals.reduce((total, goal) => {
      const weightage = parseInt(goal.weightage) || 0;
  ...
  const validateGoals = () => {
    ...
    goals.forEach((goal, index) => {
      ...
      if (!goal.title || !goal.title.trim()) {
        goalErrors.title = "Goal/Objective is required";
        isValid = false;
      }

      const weightage = parseInt(goal.weightage);
      if (isNaN(weightage)) {
        goalErrors.weightage = "Weightage is required";
        isValid = false;
      } else if (weightage < 0 || weightage > 100) {
        goalErrors.weightage = "Weightage must be between 0 and 100";
        isValid = false;
      } else if (weightage === 0) {
        goalErrors.weightage = "Weightage cannot be 0%";
        isValid = false;
      }
    ...
    if (totalWeightage !== 100) {
      setErrors((prev) => ({
        ...prev,
        totalWeightage: `Total weightage must be 100%. Current total: ${totalWeightage}%`,
      }));
      isValid = false;
    } else {
      setErrors((prev) => {
        const newPrev = { ...prev };
        delete newPrev.totalWeightage;
        return newPrev;
      });
    }
  ...
  const saveGoal = async (goal, isDraft = false) => {
    const storedEmpId = localStorage.getItem("empId") || empId;
    ...
    const payload = {
      ...
      weightage: parseInt(goal.weightage) || 0,
    };
    ...
    if (goal.id) {
      return await axios.put(
        `${BASE_URL_EPMS}/api/goals/draft/${goal.id}`,
        payload,
      );
    } else {
      if (isDraft) {
        return await axios.post(
          `${BASE_URL_EPMS}/api/goals/draft/${selectedQuarter}`,
          payload,
        );
      } else {
        return await axios.post(
          `${BASE_URL_EPMS}/api/goals/create/${selectedQuarter}`,
          payload,
        );
      }
    }
  ...
  const handleSaveAsDraft = async () => {
    ...
    try {
      for (const goal of goals) {
        await saveGoal(goal, true);
      }
      showModal(
        "Success",
        "SMART Goals saved as draft successfully!",
        "success",
        () => {
          closeModal();
          navigate(-1);
      ...
      console.error("Error saving goals as draft:", err);
      const errorMsg =
  ...
  const handleSubmitForApproval = async () => {
    if (!validateGoals()) {
      showModal(
        "Validation Error",
        "Please fill all required fields and ensure total weightage is 100%",
        "error",
      );
      return;
    }
    ...
    showModal(
      "Confirm Submission",
      "Are you sure you want to submit these goals for manager approval?",
      "warning",
      async () => {
        setSubmitting(true);
        try {
          ...
          showModal(
            "Success",
            "SMART Goals submitted for manager approval successfully!",
            "success",
            () => {
              closeModal();
              navigate(-1);
          ...
          const errorMsg =
            err.response?.data?.message ||
            err.message ||
            "Failed to submit goals. Please try again.";
          showModal("Error", errorMsg, "error");
        } finally {
          setSubmitting(false);
        }
  ...
  // Development Goals Table Component
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-red-100 text-sm mt-1">
                Add your training and development goals for the quarter
              </p>
            </div>
            <button
              onClick={() => handleOpenDevModal()}
              className="px-4 py-2 bg-white/20 text-white text-sm rounded-lg hover:bg-white/30 transition-colors flex items-center gap-2"
            >
          ...
          {developmentGoals.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">
                Click "Add Development Goal" to add your training and
                development plans.
              </p>
            </div>
          ) : (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex gap-2 justify-center">
                          ...
                          <button
                            onClick={() => deleteDevelopmentGoal(goal.id)}
      ...
      <div className="mt-24 px-4 md:px-8 max-w-7xl mx-auto w-full pb-12">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500 mt-1 ml-7">
            Define your SMART goals and development objectives for the upcoming
            quarter
          </p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                ...
                <div>
                  ...
                  <p className="font-semibold text-gray-800">
                    {getManagerFullName(employeeData)}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-md overflow-hidden border border-gray-100">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                ...
                <p className="text-gray-300 text-sm mt-1">
                  Add your SMART goals for {selectedQuarter} {selectedYear}
                </p>
              </div>
          ...
          <div className="p-6">
            {goals.map((goal, index) => (
              <div
                key={index}
                className="mb-6 bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow overflow-hidden"
              >
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div>
                    ...
                    {errors[index]?.title && (
                      <p className="text-xs text-red-500 mt-1 flex items-center gap-1">
                        <FaInfoCircle /> {errors[index].title}
                      </p>
                    )}
                  </div>

                  {/* Target - Dropdown only */}
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">
                      <FaClipboardList className="inline mr-1 text-red-500 text-xs" />
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all appearance-none bg-white"
                      >
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">
                      <FaPercent className="inline mr-1 text-red-500 text-xs" />
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        onChange={(e) =>
                          handleGoalChange(index, "weightage", e.target.value)
                        }
                        placeholder="0"
                        min="0"
                        max="100"
                    ...
                    {errors[index]?.weightage && (
                      <p className="text-xs text-red-500 mt-1 flex items-center gap-1">
                        <FaInfoCircle /> {errors[index].weightage}
                      </p>
                    )}
                  </div>
                </div>
            ...
            <button
              onClick={addNewGoal}
              className="w-full py-3 border-2 border-dashed border-red-300 text-red-600 rounded-xl hover:bg-red-50 transition-all flex items-center justify-center gap-2 font-medium group"
            >
        ...
        <div className="flex flex-col sm:flex-row justify-end gap-3 pt-8 mt-4">
          ...
          <button
            ...
            disabled={submitting || !hasEnteredAnything}
            className="px-6 py-2.5 bg-amber-500 text-white rounded-xl hover:bg-amber-600 transition-colors flex items-center justify-center gap-2 font-medium disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            disabled={submitting || !hasEnteredAnything}
            className="px-6 py-2.5 bg-green-600 text-white rounded-xl hover:bg-green-700 transition-colors flex items-center justify-center gap-2 font-medium disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg"
          >
            {submitting ? (
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-xl border border-blue-100">
          <p className="text-sm text-blue-700 flex items-start gap-2">
            ...
            <span>
              <strong>SMART Goals:</strong> Total weightage must equal 100%
              before submission. <strong>Development Goals:</strong> These are
              for training and skill development purposes. Self assessment will
              be enabled after manager approval.
            </span>
          </p>
        </div>
      </div>

      {/* Development Goal Modal */}
      <DevelopmentGoalModal
        isOpen={showDevGoalModal}
        onClose={handleCloseDevModal}
        onSave={saveDevelopmentGoal}
        editingGoal={editingDevGoal}
        devGoalForm={devGoalForm}
        setDevGoalForm={setDevGoalForm}
        devErrors={devErrors}
...
export default EmployeeGoalCreation;