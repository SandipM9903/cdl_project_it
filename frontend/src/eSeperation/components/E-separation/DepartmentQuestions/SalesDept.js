...
function SalesDept({ selectedEmployee, onClose }) {
    ...
    const validate = () => {
        ...
        if (salesData.customerList === "") {
            errors.customerList = 'Field is required';
            isValid = false;
        }

        if (salesData.billingDetails === "") {
            errors.billingDetails = 'Field is required';
            isValid = false;
        }

        if (salesData.pendingSOF === "") {
            errors.pendingSOF = 'Field is required';
            isValid = false;
        }
        if (salesData.customerDatabase === "") {
            errors.customerDatabase = 'Field is required';
            isValid = false;
        }
        if (salesData.ordersPipeline === "") {
            errors.ordersPipeline = 'Field is required';
            isValid = false;
        }
        if (salesData.ars === "") {
            errors.ars = 'Field is required';
            isValid = false;
        }
        if (salesData.salesInCharge.trim() === "") {
            errors.salesInCharge = 'Field is required';
            isValid = false;
        }
        if (salesData.salesRemarks.trim() === "") {
            errors.salesRemarks = 'Field is required';
            isValid = false;
        }
    ...
    useEffect(() => {
        axios.get(`${BASE_URL}:9029/api/eSeparation/getQuestionsByDept/Sales`)
            .then((res) => {
                const department = res.data.find(dept => dept.deptName === 'Sales');
                if (department) {
    ...
    const handleApprove = () => {
        ...
        axios.post(`${BASE_URL}:9029/api/eSeparation/fillSalesForm/${selectedAction}`, data)
            .then(() => {
                console.log("Posted");
                window.location.reload();
            })
    ...
    return (
        ...
        <h1 className='text-gray-800 font-semibold mx-5 mt-3'>To be filled by Sales Department</h1>
                    ...
                    {salesQuestions.map((question, index) => (
                        <div key={question.questionId} className='grid grid-cols-3 mt-3'>
                            ...
                            <div className='FormDropdown'>
                                {index === 0 && (
                                    <>
                                    <Dropdown className='w-60' placeholder={"Select"} options={statusDropdown} onChange={handleDropdownChange('customerList')} />
                                    {errors.customerList && <p className='text-red-500 font-semibold text-[10px] mt-1'>{errors.customerList}</p>}
                                    </>
                                )}
                                {index === 1 && (
                                    <>
                                    <Dropdown className='w-60' placeholder={"Select"} options={statusDropdown} onChange={handleDropdownChange('billingDetails')} />
                                    {errors.billingDetails && <p className='text-red-500 font-semibold text-[10px] mt-1'>{errors.billingDetails}</p>}
                                    </>
                                )}
                                {index === 2 && (
                                    <>
                                    <Dropdown className='w-60' placeholder={"Select"} options={statusDropdown} onChange={handleDropdownChange('pendingSOF')} />
                                    {errors.pendingSOF && <p className='text-red-500 font-semibold text-[10px] mt-1'>{errors.pendingSOF}</p>}
                                    </>
                                )}
                                {index === 3 && (
                                    <>
                                    <Dropdown className='w-60' placeholder={"Select"} options={statusDropdown} onChange={handleDropdownChange('customerDatabase')} />
                                    {errors.customerDatabase && <p className='text-red-500 font-semibold text-[10px] mt-1'>{errors.customerDatabase}</p>}
                                    </>
                                )}
                                {index === 4 && (
                                    <>
                                    <Dropdown className='w-60' placeholder={"Select"} options={statusDropdown} onChange={handleDropdownChange('ordersPipeline')} />
                                    {errors.ordersPipeline && <p className='text-red-500 font-semibold text-[10px] mt-1'>{errors.ordersPipeline}</p>}
                                    </>
                                )}
                                {index === 5 && (
                                    <>
                                    <Dropdown className='w-60' placeholder={"Select"} options={statusDropdown} onChange={handleDropdownChange('ars')} />
                                    {errors.ars && <p className='text-red-500 font-semibold text-[10px] mt-1'>{errors.ars}</p>}
                                    </>
                                )}
                                {index === 6 && (
                                    <>
                                    <input type="text" name="salesInCharge" className='h-8 outline-none ml-2 rounded-md py-[1px] px-1 border border-gray-400 w-56 text-sm' onChange={handleChange} />
                                    {errors.salesInCharge && <p className='text-red-500 font-semibold text-[10px] mt-1'>{errors.salesInCharge}</p>}
                                    </>
                                )}
                                {index === 7 && (
                                    <>
                                    <textarea name="salesRemarks" rows={3} className={`FormTextArea ml-2 outline-none rounded-md py-[1px] px-1 border border-gray-400 w-56 text-sm ${errors.salesRemarks ? '' : 'mb-6'}`} onChange={handleChange} />
                                    {errors.salesRemarks && <p className='text-red-500 font-semibold text-[10px] mt-1'>{errors.salesRemarks}</p>}
                                    </>
                                )}
                            </div>
                        </div>
