...
function ExitedEmployees() {
  const [empDetails, setEmpDetails] = useState([]);
  const [deptDetails, setDeptDetails] = useState(null); // 🔹 separate state for dept export details
      ...
      .then((res) => {
        ...
        Promise.all(employeeDetailPromises)
          .then((responses) => {
            ...
            // Call getImage for each empId in mergedData
            //  mergedData.forEach(empDetail => getImage(empDetail.empId));
            // 🔹 Call dept export API separately using wfSeqId
            // 🔹 Fetch dept details for each wfSeqId and merge
            const deptDetailPromises = mergedData.map((data) =>
              axios
                .get(
                  `${BASE_URL}:9029/api/eSeparation/getAllDeptDetailsForExport/${data.wfSeqId}`
                )
                .then((deptRes) => ({
                  ...data,
                  deptDetails: deptRes.data, // attach dept details here
                }))
            );

            Promise.all(deptDetailPromises)
              .then((finalData) => {
                setEmpDetails(finalData);
                console.log("Final mergedData with dept details:", finalData);
              })
              .catch((error) => {
  ...
  const cleanClearance = (value) => {
    ...
    // ensure string, remove leading number+dot, trim and lowercase
    const status = String(value).replace(/^\d+\./, "").trim().toLowerCase();

    if (status.startsWith("approve")) return "Approved";
    if (status.startsWith("reject")) return "Rejected";


    // Generic fallback: capitalize first letter of each word
    return status
      .split(/\s+/)
  ...
  const exportToExcel = () => {
    ...
    const dataForExcel = empDetails.map((employee, index) => ({
      "S.No": index + 1,
      "Employee Code": employee?.fileAndObjectTypeBean?.empResDTO?.empCode || "-",
      "Employee Name": employee?.fileAndObjectTypeBean?.empResDTO?.fullNameAsAadhaar || "-",
      "Designation":
        employee?.fileAndObjectTypeBean?.empResDTO?.designationResDTO?.designationName ||
        "-",
      "Date of Joining": formatDate(
        employee?.fileAndObjectTypeBean?.empResDTO?.dateOfJoining
      ),
      "Date of Resignation": formatDate(employee?.dateOfResignation),
      "Last Working Date": formatDate(employee?.lastWorkingDay),
      "Requested Last Working Date": formatDate(employee?.lastWorkingDayRequest),
      "Reason": employee?.reason || "-",
      "Remarks": employee?.remarks || "-",
      "R1 Status": employee?.r1Status || "-",
      "HR Status": employee?.hrStatus || "-",

      "Payroll": employee?.fileAndObjectTypeBean?.categoryResDTO?.category || "-",
      "Dept": employee?.fileAndObjectTypeBean?.mainDeptResDTO?.mainDepartment || "-",
      "Sub Dept": employee?.fileAndObjectTypeBean?.subDeptResDTO?.subDept || "-",
      // 🔽 Extra Fields (placeholders for you to map)
      "Exit Type": employee?.exitType || "-",
      "Location": "-",
      "Reporting Manager": employee?.fileAndObjectTypeBean?.empResDTO?.reportingManager || "-",
      "R1 Approval Status": employee?.r1Status || "-",
      "HR Approval Status": employee?.hrStatus || "-",
      "Notice Period Required as per policy": "90 days",
      "Notice Period Served": employee?.noticePeriodWaiver === false
        ? "Yes" : employee?.noticePeriodWaiver === true ? "No" : "-",
      "Shortfall of Notice Period": "-",
      "Recovery (If applicable)": "-",
      "Notice Recovery Recovery Amt": "-",
      "Exit form Status": employee?.overAllStatus || "-",
      "Exit form submission date": employee?.createdDate || "-",
      "KT Dept Clearance": cleanClearance(employee?.deptDetails?.ktDeptClearance) || "-",
      "Approver Name (KT Dept)": employee?.deptDetails?.ktApproverName || "-",
      "KT Dept Clearance Date": employee?.deptDetails?.ktClearanceDate || "-",

      "IT Dept Clearance": cleanClearance(employee?.deptDetails?.itDeptClearance) || "-",
      "Approver Name (IT Dept)": employee?.deptDetails?.itApproverName || "-",
      "IT Dept Clearance Date": employee?.deptDetails?.itClearanceDate || "-",

      "Admin Dept Clearance": cleanClearance(employee?.deptDetails?.adminDeptClearance) || "-",
      "Approver Name (Admin Dept)": employee?.deptDetails?.adminApproverName || "-",
      "Admin Dept Clearance Date": employee?.deptDetails?.adminClearanceDate || "-",

      "Sales Dept Clearance": cleanClearance(employee?.deptDetails?.salesDeptClearance) || "-",
      "Approver Name (Sales Dept)": employee?.deptDetails?.salesApproverName || "-",
      "Sales Dept Clearance Date": employee?.deptDetails?.salesClearanceDate || "-",

      "Store Dept Clearance": cleanClearance(employee?.deptDetails?.storeDeptClearance) || "-",
      "Approver Name (Store Dept)": employee?.deptDetails?.storeApproverName || "-",
      "Store Dept Clearance Date": employee?.deptDetails?.storeClearanceDate || "-",

      "Finance Dept Clearance": cleanClearance(employee?.deptDetails?.financeDeptClearance) || "-",
      "Approver Name (Finance Dept)": employee?.deptDetails?.financeApproverName || "-",
      "Finance Dept Clearance Date": employee?.deptDetails?.financeClearanceDate || "-",

      "BU Dept Clearance": cleanClearance(employee?.deptDetails?.buDeptClearance) || "-",
      "Approver Name (BU Dept)": employee?.deptDetails?.buApproverName || "-",
      "BU Dept Clearance Date": employee?.deptDetails?.buClearanceDate || "-",

      "HR Dept Clearance": cleanClearance(employee?.deptDetails?.hrDeptClearance) || "-",
      "Approver Name (HR Dept)": employee?.deptDetails?.hrApproverName || "-",
      "HR Dept Clearance Date": employee?.deptDetails?.hrClearanceDate || "-",
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Exited Employees");
    ...
    saveAs(data, "ExitedEmployeesReport.xlsx");
  };
  ...
  return (
    <div className="container mx-auto">
      <div className="flex justify-end">
        <div className="flex justify-center gap-2 mt-6 mr-80">
          <input
            type="text"
        ...
        <button
          ...
          className="bg-blue-600 text-white px-4 py-2 rounded-lg shadow-md mt-6 hover:bg-blue-700"
        >
          Export Excel
        </button>
      </div>
      ...
      {empDetails.length === 0 ? (
        ...
        filteredEmployeeList.map((employee) => (
          <div
            key={employee.id}
            className="grid lg:grid-cols-6 md:grid-cols-3 grid-cols-2 mx-8 mt-6 border rounded-lg border-gray-500 p-2 shadow-bottom"
          >
            ...
            <div className="col-span-1 lg:-ml-2 lg:w-52 md:w-full w-full">
              ...
              <div className="">
                ...
                <div className="grid grid-cols-2  text-sm text-gray-700 p-2">
                  <h1 className="text-xs">{getLabel(employee)}</h1>
            ...
            <div className="col-span-1 lg:ml-11 md:ml-0 ml-0 lg:w-32 w-full md:w-full">
              ...
              <h1 className="text-gray-900 p-1 px-3 font-semibold mt-5 text-sm lg:mx-6 md:mx-12 mx-6 mr-6 inline-block rounded-md bg-[#A8ED87] text-black py-[6px]">
                {employee?.r1Status}
              </h1>
            </div>
            ...
            <div className="col-span-1 lg:w-48 md:w-full w-full">
              <h1 className="text-gray-600 font-semibold mx-10">Exit Status</h1>
              <h1
                className={`text-gray-900 p-1 px-3 font-semibold mt-5 text-sm lg:mx-10 md:mx-12 mx-6 mr-6 inline-block rounded-md bg-[#A8ED87] lg:mt-[38px] text-black py-[6px]`}
              >
