...
function PendingTab({ employeeDetails, resignationDetails }) {
  const [resignData] = resignationDetails.filter(
    (detail) =>
      detail.overAllStatus !== "Closed" && detail.overAllStatus !== "Withdrawn"
  );
  ...
  //     axios
  //       .get(`${BASE_URL}:9029/api/eSeparation/download?empCode=${empCode}`, {
  ...
  const empDetail =
    resignData && resignationDetails.length > 0
      ? resignationDetails.find((detail) => detail.id === resignData.id) // Correct comparison
      : null;
  ...
  const fetchComment = async () => {
    try {
      const response = await axios.get(
        `${BASE_URL}:9029/api/eSeparation/getEmpComments/${resignData.empCode}`
      );
  ...
  const postComment = () => {
    ...
    const url = `${BASE_URL}:9029/api/eSeparation/sendEmpComments/${resignData.empCode}`;
  ...
  const handleWithdrawExit = async () => {
    const url = `${BASE_URL}:9029/api/eSeparation/withdraw/${resignData.empCode}`;
    ...
    <div className="container mx-auto">
      <div className="grid grid-cols-3">
        <div className="col-span-2 py-4 px-2">
          <div className="grid grid-cols-12 bg-white border border-gray-300 shadow-bottom">
            <div className="col-span-3 text-center">
              ...
              <h1 className="text-[#1B5CAD] text-sm font-semibold">
                {employeeDetails?.fileAndObjectTypeBean?.empResDTO?.fullNameAsAadhaar ||
                  "NA"}
              </h1>
              <h1 className="text-[#8A7C7C] text-xs">
                {employeeDetails?.fileAndObjectTypeBean?.empResDTO?.designationResDTO?.designationName || "NA"}
              </h1>
            </div>
            <div className="col-span-9 ml-8">
              <div className="grid grid-cols-3 text-xs mt-2 gap-2 mx-4">
                <div>
                  <h1 className="text-[#8A7C7C] mt-5">Employee ID</h1>
                ...
                <div>
                  <h1 className="text-[#8A7C7C] mt-5">Contact</h1>
                  <h1 className="text-[#021C59] mt-1">
                    {employeeDetails?.fileAndObjectTypeBean?.empResDTO?.primaryContactNo || "NA"}
                  </h1>
                </div>
                <div>
                  <h1 className="text-[#8A7C7C] mt-5">Email</h1>
              ...
              <div className="grid grid-cols-3 mt-2 text-xs gap-2 mx-4">
                <div>
                  <h1 className="text-[#8A7C7C] mt-3">Department</h1>
                  <h1 className="text-[#021C59] mt-1">
                    {employeeDetails?.fileAndObjectTypeBean?.empResDTO?.mainDeptResDTO.mainDepartment || "NA"}
                  </h1>
                </div>
                <div>
                  <h1 className="text-[#8A7C7C] mt-3">Reporting To</h1>
                  <h1 className="text-[#021C59] mt-1">
                    {employeeDetails?.fileAndObjectTypeBean?.empResDTO?.reportingManager || "NA"}
                  </h1>
                </div>
                <div>
                  <h1 className="text-[#8A7C7C] mt-3">Employment Type</h1>
                  <h1 className="text-[#021C59] mt-1">
              ...
              <div className="grid grid-cols-3 mt-2 text-xs gap-2 mx-4">
                <div>
                  <h1 className="text-[#8A7C7C] mt-3">Location</h1>
                  <h1 className="text-[#021C59] mt-1">
                ...
                {/* <div>
                  <h1 className="text-[#8A7C7C] mt-3">Empo</h1>
                  <h1 className="text-[#021C59] mt-1">
              ...
              <div className="grid grid-cols-3 mt-2 text-xs gap-2 mx-4">
                <div>
                  <h1 className="text-[#8A7C7C] mt-3">Joining Date</h1>
                ...
                <div>
                  <h1 className="text-[#8A7C7C] mt-3">Project/Cost Centre</h1>
                  <h1 className="text-[#021C59] mt-1 mb-1">
                    {employeeDetails?.fileAndObjectTypeBean?.empResDTO?.projectResDTO?.projectName || "NA"}
                  </h1>
                </div>
              </div>
            </div>
          </div>
          <div className="bg-white grid mt-4 grid-cols-1 text-xs border border-gray-300 shadow-bottom">
            <div className="grid grid-cols-3 mx-10">
              <div>
                <h1 className="text-[#8A7C7C] mt-4">Date of Resignation</h1>
                <h1 className="text-gray-800 mt-1">
                  {formatDate(resignData?.dateOfResignation) || "-"}
                </h1>
              </div>
              <div>
                <h1 className="text-[#8A7C7C] mt-4">Reason for Exit</h1>
                <h1 className="text-gray-800 mt-1">
                  {resignData?.reason || "-"}
                </h1>
              </div>
              <div>
                <h1 className="text-[#8A7C7C] mt-3">Notice Period</h1>
                <h1 className="text-gray-800 mt-1">{noticePeriod} Days</h1>
              </div>
            </div>
            <div className="grid grid-cols-3 mx-10">
              <div>
                <h1 className="text-[#8A7C7C] mt-5">
        ...
        <div className="col-span-1 bg-white border border-gray-300 mt-4 mb-4">
          <div className="">
            <div className="ml-3 border-gray-300">
              <h2 className="mb-6 text-base font-semibold ml-4 mt-2">Discussion Thread</h2>
              <div className="flex justify-between items-center p-4">
                <div>
                  <h2 className="text-sm text-[#4B4C4D] font-medium">{resignData.overAllStatus === 'Submitted to Manager'
                    ...
                    : resignData.overAllStatus === 'Pending With HR' || resignData.overAllStatus === 'Not Started'|| resignData.overAllStatus === 'Interview Initiated'
