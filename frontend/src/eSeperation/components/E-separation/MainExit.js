...
import InitiateExitInterview from "./InitiateExitInterview";
import AllRequestsDisplayForHr from "./AllRequestsDisplayForHr"
...
function MainExit() {
  ...
  const [activeTab, setActiveTab] = useState("allRequestsDisplayForHr");
  ...
  const fetchResignData = () => {
    axios
      .get(`${BASE_URL}:9029/api/eSeparation/getResignDataByHrEmpId/${empCode}`)
      .then((response) => {
  ...
  const handleEmployeeClick = (employee) => {
    ...
    setEmployeeSearch(employee?.fileAndObjectTypeBean?.empResDTO?.firstName);
  };
  ...
  const filteredEmployees = empDetails.filter((emp) => {
    ...
    const searchMatches =
      employeeSearch &&
      (
        emp.fileAndObjectTypeBean.empResDTO.empCode.toString() === employeeSearch ||
        (emp.fileAndObjectTypeBean.empResDTO.firstName &&
          emp.fileAndObjectTypeBean.empResDTO.firstName
            .toLowerCase()
  ...
  const handleSubmit = () => {
    ...
    axios
      .post(
        `${BASE_URL}:9029/api/eSeparation/initiateExitByHR/${selectedEmployee?.fileAndObjectTypeBean?.empResDTO?.empCode}/${workFlowName}`,
        formData
      )
      .then((res) => {
  ...
  const validateFields = () => {
    ...
    if (!resignData?.dateOfResignation) {
      errors.dateOfResignation = "Date of Exit is required";
      isValid = false;
    } else {
      errors.dateOfResignation = "";
    }
    if (!resignData?.exitType) {
      errors.reason = "Exit Type is required";
      isValid = false;
    } else {
      errors.exitType = "";
    }
  ...
  return (
    <div className="h-screen ]">
      ...
      <div className=" mt-20 bg-white ">
        <div className="flex lg:flex-row md:flex-row flex-col border-b border-gray-300 sticky top-0 bg-white z-50">
          ...
          <div className="mx-auto text-xs p-4 lg:mt-5 md:mt-5 mt-0 ">
            <ul className="flex lg:-ml-36 md:-ml-20 ml-0 lg:space-x-6 md:space-x-2 space-x-0 ">
              ...
              <li className="me-2">
                <button
                  href="/allRequestsDisplayForHr"
                  type="button"
                  onClick={() => setActiveTab("allRequestsDisplayForHr")}
                  className={`inline-block p-2 border-b-2 font-bold ${activeTab === "allRequestsDisplayForHr"
                    ? "lg:border-blue-600 md:border-blue-600 border-transparent text-blue-600"
                    : "border-transparent hover:text-gray-900"
                    }`}
                >
              ...
              <li className="me-2">
                <button
                  href="/initiateExitInterview"
                  type="button"
                  onClick={() => setActiveTab("initiateExitInterview")}
                  className={`inline-block p-2 border-b-2 font-bold ${activeTab === "initiateExitInterview"
                    ? "lg:border-blue-600 md:border-blue-600 border-transparent text-blue-600"
                    : "border-transparent hover:text-gray-900"
                    }`}
                >
            ...
            <div className="grid lg:grid-cols-2 md:grid-cols-2 grid-cols-1">
              <div className="col-span-1 lg:w-full md:w-full w-full">
                <div className="flex items-center  justify-between">
                  <h1 className="font-semibold text-gray-800 lg:ml-12 md:ml-12 ml-6 text-sm mt-6" disabled={count > 0}>
                    Initiate Exit
                  </h1>
                  {message && (
        ...
        {activeTab === "allRequestsDisplayForHr" && (
          <div>
            <AllRequestsDisplayForHr />
          </div>
        )}
        {activeTab === "initiateExitInterview" && (
          <div>
            <InitiateExitInterview />
          </div>
        )}
        {activeTab === "pending" && (
          <div>
        ...
        {activeTab === "exitProcess" && (
          <div>
            <ExitProcess />
          </div>
        )}
        {activeTab === "exitEmployees" && (
          <div>
            <ExitedEmployees />
          </div>
        )}
