...
const Login = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}:9028/api/workflow/loginuser/${userId}/${password}`);
      // Assign value to a key
      console.error('Login Response:', response.data.empCode);
     
        sessionStorage.setItem('UserId', response.data.empCode);
  ...
  return (
    ...
    <div className="bg-gray-600 bg-blend-multiply bg-center bg-no-repeat bg-[url('https://candid.technology/wp-content/uploads/2019/02/Presentation-office-909013.jpg')] bg-cover ">
      <div className="flex justify-center items-center h-screen ">
        ...
        <div className="w-1/2 text-center ">
          ...
          <p className="mb-8 text-lg font-bold text-gray-300 lg:text-2xl sm:px-16 lg:px-49">
            To keep connected with <span className="text-red-500">CMS Digital Lounge</span>, please login with your
            Email and Password
          </p>
        </div>
        {/* Right Div */}
        <div className="w-1/3 rounded overflow-hidden bg-gray-400 bg-opacity-60 p-6 shadow-2xl">
          <div className="flex justify-center items-center">
            <span className="">
              <img src={human} alt="Count Icon 1" className="rounded-full  w-16 h-16 mb-4" />
            </span>
          </div>
          <form>
            ...
            <a href="https://www.example.com" className='text-white'>Forgot Password ?</a>
          </form>
